...
public class Bank {
    ...
    int balance = 0;
    final Lock balanceLock = new ReentrantLock();
    ...
    // A deposit function that will run in parallel on a separate thread. It will be
    // a loop where in each iteration, it read the amount from the operationQueue
    ...
    // A withdraw function that will run in parallel on a separate thread. It will
    // be a loop where in each iteration, it read the amount from the operationQueue
